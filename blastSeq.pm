...
sub work {
	...
	my $statement = qq{UPDATE blastSeqDoglas SET jobstatus = "R" where jobid = ?};
	...
	system("blastp -db ".$db." -query ".$queryFile." -outfmt 6 > /var/www/html/disciplina/webdev20171/doglas/result/".$jobID."_result.txt");
	system("rm $queryFile");

	# change job status to Complete
	my $status = "C";
	my $statement2 = qq{UPDATE blastSeqDoglas SET jobstatus = ? where jobid = ?};
